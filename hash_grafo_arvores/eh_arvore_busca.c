...
bool eBinariaBusca(ARVORE_BINARIA *arvore)
{
    ...
    if (!vazia(arvore->esq))
    {
        ARVORE_BINARIA *maxEsq = arvore->esq;
        while (!vazia(maxEsq->dir))
        {
            maxEsq = maxEsq->dir;
        }
        
        if (maxEsq->item.chave >= arvore->item.chave)
        {
            return false;
        }
    }
    ...
    if (!vazia(arvore->dir))
    {
        ARVORE_BINARIA *minDir = arvore->dir;
        
        while (!vazia(minDir->esq))
        {
            minDir = minDir->esq;
        }

        if (minDir->item.chave <= arvore->item.chave)
        {
            return false;
        }
    }

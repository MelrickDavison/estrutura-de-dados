...
bool buscaValor(ARVORE_BINARIA *atual, TIPOCHAVE chave, ARVORE_BINARIA *noOriginal)
{
    if (atual == NULL) {
        return false;
    }

    if (atual->item.chave == chave && atual != noOriginal) {
        return true;
    }

    return buscaValor(atual->esq, chave, noOriginal) || 
           buscaValor(atual->dir, chave, noOriginal);
}
...
bool haDuplicados(ARVORE_BINARIA *arvore, ARVORE_BINARIA *raiz)
{
    ...
    if (buscaValor(raiz, arvore->item.chave, arvore)) {
        return true;
    }

    return haDuplicados(arvore->esq, raiz) || 
           haDuplicados(arvore->dir, raiz);
}

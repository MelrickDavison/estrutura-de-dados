...
bool copiarArvore(ARVORE_BINARIA *origem, ARVORE_BINARIA **destino) 
{
    if (origem == NULL) 
        return true;
    if (!inserir(origem->item, destino)) 
        return false;
        
    if (!copiarArvore(origem->esq, destino)) 
        return false;
        
    if (!copiarArvore(origem->dir, destino)) 
        return false;
 ...
 *arvU = NULL;
    ...
    if (!copiarArvore(arv1, arvU)){
        disposeArvore(*arvU);
        *arvU = NULL;
        return false;
    }

    if (!copiarArvore(arv2, arvU)){
        disposeArvore(*arvU);
        *arvU = NULL;
        return false;
    }
